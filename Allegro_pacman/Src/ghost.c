...
void ghost_draw(Ghost* ghost) {
	...
	if (ghost->status == FLEE) {
		...
		if (al_get_timer_count(power_up_timer) <= 7) {
			...
			if (ghost->objData.moveCD % 32 < 8) {
				al_draw_scaled_bitmap(ghost->flee_sprite, 0, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,draw_region, draw_region, 0);
			}
			else if (ghost->objData.moveCD % 32 < 16) {
			...
			else if (ghost->objData.moveCD % 32 < 24) {
				al_draw_scaled_bitmap(ghost->flee_sprite, 16, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			else if (ghost->objData.moveCD % 32 < 8) {
		...
		case RIGHT:
			if (ghost->objData.moveCD % 64 <= 32) {
				...
				al_draw_scaled_bitmap(ghost->move_sprite, 16, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			break;
		case UP:
			if (ghost->objData.moveCD % 64 <= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 80, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			else if (ghost->objData.moveCD % 64 >= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 64, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			break;
		case DOWN:
			if (ghost->objData.moveCD % 64 >= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 112, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			else if (ghost->objData.moveCD % 64 <= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 96, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			break;
		case LEFT:
			if (ghost->objData.moveCD % 64 <= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 32, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			else if (ghost->objData.moveCD % 64 >= 32) {
				al_draw_scaled_bitmap(ghost->move_sprite, 48, 0, 16, 16, drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y, draw_region, draw_region, 0);
			}
			break;
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	for (Directions i = 1; i <= 4; i++) {
		if (ghost_movable(ghost, M, i, true) && i != shortestDirection && i != 5 - ghost->objData.facing) {
			proba[cnt++] = i;
		}
	}
	Directions ghostrdm;
	...
	else if (ghost->objData.facing == shortestDirection) {
		ghostrdm = shortestDirection;
	}
	else { 
		ghostrdm = 5 - ghost->objData.facing; 
	}
