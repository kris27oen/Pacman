	...
	*/
    ...
    else if (cnt < 3)
    {
        if ((ghost_movable(ghost, M, ghost->objData.facing, true)))
            ghost_NextMove(ghost, ghost->objData.facing);
        else{
        ...
        switch (ghost->objData.facing){
        case UP:
            if (rando == 1 && ghost_movable(ghost, M, DOWN, true)){
                ghost_NextMove(ghost, DOWN);
                break;
            }
            else{
                while (1){
                    Directions inside = rand() % 4 + 1;
                    if (ghost_movable(ghost, M, inside, true) && inside != DOWN){
                        ghost_NextMove(ghost, inside);
                        break;
                    }
                }
            }
            break;
        case DOWN:
            if (rando == 1 && ghost_movable(ghost, M, UP, true)){
                ...
                while (1){
                    Directions inside = rand() % 4 + 1;
                    if (ghost_movable(ghost, M, inside, true) && inside != UP){
                        ghost_NextMove(ghost, inside);
                        break;
                    }
                }
            ...
            break;
        case LEFT:
            if (rando == 1 && ghost_movable(ghost, M, RIGHT, true)){
                ...
                while (1){
                    Directions inside = rand() % 4 + 1;
                    if (ghost_movable(ghost, M, inside, true) && inside != RIGHT){
                        ghost_NextMove(ghost, inside);
                        break;
                    }
                }
            ...
            break;
        case RIGHT:
            if (rando == 1 && ghost_movable(ghost, M, LEFT, true)){
                ...
                while (1){
                    Directions inside = rand() % 4 + 1;
                    if (ghost_movable(ghost, M, inside, true) && inside != LEFT){
                        ghost_NextMove(ghost, inside);
                        break;
                    }
                }
...
void ghost_orange_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			ghost_red_move_script_BLOCKED(ghost, M);
			if (al_get_timer_count(game_tick_timer) > GO_OUT_TIME*6)
				ghost->status = GO_OUT;
			break;
		case FREEDOM:
			ghost_red_move_script_FREEDOM(ghost, M);
