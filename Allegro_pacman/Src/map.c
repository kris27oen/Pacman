...
static void draw_poison_bean(Map* M, const int row, const int col);
static void draw_ghost_die_bean(Map* M, const int row, const int col);
	...
	for (int row = 0; row < M->row_num; row++) {
		for (int col = 0; col < M->col_num; col++) {
			...
			case '.':
				...
				break;
			case 'S':
				draw_poison_bean(M, row, col);
				break;
			case 'G':
				draw_ghost_die_bean(M, row, col);
				break;
			default:
				break;
...
static void draw_poison_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(30, 0, 60));
}
static void draw_ghost_die_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(255, 255, 255));
}
