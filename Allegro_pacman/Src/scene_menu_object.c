...
void drawButton(Button button) {
	ALLEGRO_BITMAP* _img = button.hovered_img ? button.hovered ? button.hovered_img : button.default_img : button.default_img;
...
bool buttonHover(Button button, int mouse_x, int mouse_y) {
	// Check if mouse is hovering on the button
